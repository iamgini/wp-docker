...
//        );
                ...
                [
                    ...
                    'raw'  => '
						...
						<div class="mailoptin-panel-nerd-box-message">' .
                              sprintf(
                                  __('Upgrade to Premium to support custom fields data besides name and email and remove the 500 subscribers per month limit.', 'mailoptin'),
                                  '<strong>',
                                  '</strong>'
                              ) .
                              '</div>
    ...
    public function run($record, $ajax_handler)
    {
        ...
        if (defined('MAILOPTIN_DETACH_LIBSODIUM') && is_array($fields_map) && ! empty($fields_map)) {
            $optin_data->payload['form_custom_field_mappings'] = [];
            foreach ($fields_map as $mapped_field) {
                $optin_data->payload['form_custom_field_mappings'][$mapped_field['remote_id']] = $mapped_field['local_id'];
            }
        }
